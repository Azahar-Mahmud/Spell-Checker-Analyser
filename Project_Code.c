...
int main() {
    ...
    fprintf(csv, "Words,Total Runtime (ms)\n");
    fclose(csv);

    csv = fopen("lcs_time.csv", "w");
    fprintf(csv, "Words,LCS Runtime (ms)\n");
    fclose(csv);

    csv = fopen("string_match_time.csv", "w");
    fprintf(csv, "Words,String Matching Runtime (ms)\n");
    fclose(csv);

    csv = fopen("knapsack_time.csv", "w");
    fprintf(csv, "Words,Knapsack Runtime (ms)\n");
    fclose(csv);
    ...
    for (int i = 1; i <= 10; i++) {
        ...
        double total_time = (lcs_total + string_match_total + knapsack_total);

        fprintf(outputFile, "\nResults for %d words:\n", current_limit);
        fprintf(outputFile, "Total: %.4f ms | LCS: %.4f ms | String Matching: %.4f ms | Knapsack: %.4f ms\n",
                total_time, lcs_total, string_match_total, knapsack_total);
        fclose(outputFile);
        ...
        fprintf(csv, "%d,%.4f\n", current_limit, lcs_total);
        fclose(csv);

        csv = fopen("string_match_time.csv", "a");
        fprintf(csv, "%d,%.4f\n", current_limit, string_match_total);
        fclose(csv);

        csv = fopen("knapsack_time.csv", "a");
        fprintf(csv, "%d,%.4f\n", current_limit, knapsack_total);
        fclose(csv);

        printf("Results for %d words: Total: %.4f ms | LCS: %.4f ms | String Matching: %.4f ms | Knapsack: %.4f ms\n\n",
               current_limit, total_time, lcs_total, string_match_total, knapsack_total);
    }
